#!/usr/bin/env python3
"""Final verification script for RE Toolkit v1.0 - CORRECTED"""
...
# Extract results - correct structure
...
print(f"    Strings (analyzed): {strings_info.get('total', 0):,}")
print()

# Hex Editor strings (separate result)
hex_result = results.get('strings', {})
print("[+] Hex Editor Strings:")
print(f"    Total extracted: {hex_result.get('total', 0):,}")
cats = hex_result.get('categories', {})
...
# Hex editor
hex_open = agent.hex_open(agy_path)
print(f"  Hex Editor open: {hex_open.get('success', False)}")
hex_view = agent.hex_view(0, 128)
print(f"  Hex view lines: {len(hex_view.get('lines', []))}")
...
http_sessions = agent.http_get_sessions()
print(f"  HTTP sessions: {http_sessions.get('total', 0)}")
...
print(f"  JWT analysis: subject={jwt_result.get('subject', 'N/A')}, name={jwt_result.get('name', 'N/A')}")
print()

# List available tools
print("[INFO] Available AI Tools (first 20):")
tools = agent.get_available_commands()
for tool in tools[:20]:
    print(f"    - {tool}")
print(f"    ... ({len(tools) - 20} more tools)")
print()
...
print(f"  Total strings extracted: {hex_result.get('total', 0):,}")
print(f"  Analysis time: {elapsed:.1f}s")
print(f"  Memory efficient: Yes (pure Python, no external binaries)")
print()
...
# Show output files
output_dir = os.path.join(os.path.dirname(agy_path), '..', 'output')
